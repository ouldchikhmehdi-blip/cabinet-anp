...
- **Échange de colonnes verrouillé et préservé.** L'échange de deux colonnes (clic sur deux en-têtes d'associés — les initiales en haut de colonne) **verrouille** les deux colonnes et **survit à « Proposer automatiquement »**, y compris quand il porte sur une colonne **spéciale** (réa / vacances / avant-WE / après-WE) : un override valide prime sur l'occupant dérivé et est persisté. Disponible dans **toutes les vues** (aperçu déplié OU Vue continue), pas seulement la Vue continue. Les colonnes **verrouillées affichent un 🔒** dans l'en-tête de l'associé ; cliquer le 🔒 **déverrouille** la colonne (`iniVerrouilles`/`onToggleVerrou` → `basculerVerrouColonne`).
- **Résolution de l'affichage = `affectationResolue` (override + dé-doublonnage).** L'affectation affichée fusionne les colonnes spéciales dérivées (`colonnesSpeciales`) et les affectations libres stockées : (1) une affectation libre **valide** (associé **non vacancier**) sur l'index d'une colonne spéciale **override** l'occupant dérivé (échange manuel) — sans ça, l'échange « disparaissait » ; (2) aucun associé n'apparaît deux fois — un verrou **périmé** (vacancier, ou associé déjà placé) est **ignoré**, sa colonne reste vide (repourvue au prochain « Proposer »). Un override **périmé** (vacancier verrouillé sur une spéciale) ne déloge **pas** l'occupant dérivé.
- **Verrou PÉRIMÉ ignoré (pas de doublon).** Un verrou « En semaine » ne s'applique **jamais** à un associé déjà occupé cette semaine : **vacancier** (fixé à l'onglet Vacances) ou occupant d'une **colonne spéciale**. Cas typique : un associé verrouillé sur une colonne de travail puis mis **en congé** à l'onglet Vacances → le verrou est **neutralisé**, la colonne **redevient libre** (repourvue par « Proposer »), évitant qu'il occupe deux colonnes et qu'un autre finisse « non placé ». La neutralisation vaut à la fois à l'affichage (`affectationResolue` dé-doublonne) et au moteur (`proposerSemaines` : `!pris.has(v)`). Une **alerte** « verrou ignoré sur Cx » le signale ; `verrousPerimes` (invariant pur) la calcule.
